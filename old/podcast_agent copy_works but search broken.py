...
async def get_embedding(text: str, openai_client: AsyncOpenAI) -> List[float]:
    ...
    try:
        ...
        print(f"🔹 Embedding Generated for Query: {text[:100]}... -> {embedding[:5]}...")  # Log first 5 values
        return embedding
...
async def retrieve_relevant_podcast_transcripts(ctx: RunContext[PodcastAIDeps], user_query: str) -> str:
    ...
    Returns:
        A formatted string containing the most relevant transcript excerpts.
    """
    try:
        # Get query embedding
        ...
        result = ctx.deps.supabase.rpc(
            'match_podcast_transcripts',  # You'll define this function in Supabase (Step 2)
            {
        ...
        # Format the retrieved transcript chunks
        formatted_chunks = []
        for doc in result.data:
            chunk_text = f"""
**Episode {doc['episode_number']}: {doc['title']}**
...
async def test_agent():
    user_query = "Tell me about the Analytics Power Hour podcast!"
